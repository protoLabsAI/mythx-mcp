...
export const LoadWorldSummaryInputSchema = z.object({
  packId: z.string().describe("Pack ID to load"),
});

export interface LoadWorldSummaryOutput {
  packId: string;
  name: string;
  summary: string;
}

// Type for world pack structure (matches @mythxengine/worlds WorldContentPack)
interface WorldPackLike {
  meta: {
    id: string;
    name: string;
    tagline: string;
    aesthetic: { tone: string; themes: string[] };
    settings: { magicLevel: string; lethality: string };
  };
  archetypes: Record<
    string,
    {
      ...
      name: string;
      tagline: string;
      starting: { hp: number; abilities: Record<string, number> };
    }
  >;
  locations: Record<
    string,
  ...
  npcs: Record<
    string,
    { id: string; name: string; narrativeRole: string; attitude: string; locations?: string[] }
  >;
  monsters: Record<string, { id: string; name: string; threat: string; hp: number; armor: number }>;
  items: Record<string, { id: string; name: string; kind: string }>;
  encounters: Record<string, { id: string; name: string; type: string }>;
  conditions: Record<string, { id: string; name: string; duration: number | string }>;
  factions?: Record<string, { id: string; name: string; goals: string[] }>;
  situations?: Record<
    string,
    { id: string; name: string; status: string; clock?: { doom: string } }
  >;
  arcs?: Record<string, { id: string; name: string; status: string; structure: { type: string } }>;
  narrativeGuidance: { plotHooks: string[]; openingScenes: string[] };
}

/**
 * Generate a compact markdown summary of a world pack
 */
function generateWorldSummary(pack: WorldPackLike): string {
  const lines: string[] = [];
  ...
  // Narrative hooks
  if (pack.narrativeGuidance?.plotHooks?.length) {
...
export const loadWorldSummaryTool = defineSharedTool({
  name: "load_world_summary",
  description:
    "Load a compact GM reference for a world pack. Returns a token-efficient summary with entity IDs, names, and key stats. Use get_* tools for full entity details.",
  inputSchema: LoadWorldSummaryInputSchema,
  handler: async (input, ctx): Promise<LoadWorldSummaryOutput> => {
    const pack = await ctx.worldPacks.get(input.packId);
    if (!pack) {
      throw new Error(`World pack not found: ${input.packId}`);
    }

    const typedPack = pack as WorldPackLike;
    const summary = generateWorldSummary(typedPack);

    return {
      packId: input.packId,
      name: typedPack.meta.name,
      summary,
    };
  },
});
